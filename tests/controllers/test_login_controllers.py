...
class TestLoginControllers(BaseTest):
    ...
    def test_login_LDAP_invalid_form(self):
        ...
        with self.client:
            response = self.client.post(
                url_for("login"),
                data={"username": app.config.get("LDAP_USERNAME"), "password": ""},
            )
    ...
    def test_load_user(self):
        current_app.config["authentication"] = self.ldapauth
        session["_user_id"] = app.config.get("LDAP_USERNAME")
        session["user_details"] = {
            "email": "test",
            "display_name": app.config.get("LDAP_USERNAME"),
        }
        user = load_user(app.config.get("LDAP_USERNAME"))
        self.assertEqual(user.displayname, app.config.get("LDAP_USERNAME"))
        self.assertIsNone(load_user("test_none"))

    def test_save_user(self):
        session.clear()
        save_user(
            app.config.get("LDAP_USERNAME"), "test", app.config.get("LDAP_USERNAME")
        )
        self.assertEqual(
            session["user_details"].get("display_name"), app.config.get("LDAP_USERNAME")
        )
