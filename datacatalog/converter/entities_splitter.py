...
MAPPED_PROJECT_ATTRIBUTES = {"project_name": "title", "project_website": "website"}
...
def split_entities(
    ...
    for index, record in enumerate(records):
        ...
        study = Study()
        extract_entity(
            study, record, PRESERVED_STUDY_ATTRIBUTES, MAPPED_STUDY_ATTRIBUTES
        )
        ...
        extract_entity(
            project, record, PRESERVED_PROJECT_ATTRIBUTES, MAPPED_PROJECT_ATTRIBUTES
        )
        ...
        if not project.keywords:
            project.keywords = [
                random.choice(random_words) for _ in range(random.randint(1, 4))
            ]
